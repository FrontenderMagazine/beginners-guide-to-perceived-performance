...
а также несколько практических решений для ускорения работы сайта. Если вкратце, 
...
кнопки срабатывают мгновенно после нажатия на них, и пользователю не 
...
не удалось добиться желаемой скорости работы используя существующие инструменты.
...
приложении Polar][2]. Его команда приложила много усилий, чтобы уменьшить 
...
работы. В случае с индикатором, индикатор привлекал внимание к факту 
...
**К вашему вниманию четыре приёма, которые можно незамедлительно применить к делу.**
...
Браузеры добавляют задержку, чтобы убедиться, что пользователь не 
...
свидетельствует только замена фона кнопки на серый, после чего происходит 
выполнение действия. 
...
ожидания, в то время как его главная цель — быстрое взаимодействие с 
вашим сайтом.
...
мгновенное визуальное подтверждение, что что-то происходит.**

Для визуального обозначения нажатия на кнопку или ссылку есть замечательное 
состояние — `:active`, мы постоянно используем его для десктопных
браузеров. 
...
После добавления всего этого кода сразу же возникает ощущение, 
что кнопки реагируют намного быстрее, хотя по сути скорость реакции не
изменилась. Просто пользователи получают незамедлительный отклик на свои 
действия, вместо того, чтобы ждать 300 мс до какого-либо ответа.
...
Прокрутка с ускорением *кажется* вполне естественной, потому что она таковой и 
...
использования других жестов, что может оставлять у пользователей ощущение
...
увидев фотографию в сети, попробует использовать пинч, чтобы увеличить её и 
...
делаем сайты, ориентируясь на потребностей пользователей, тем скорее наступит этот день. 
