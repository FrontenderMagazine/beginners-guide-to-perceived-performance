...
This is a great example of the importance of perceived performance. It doesn’t
...
their actions instead of making them wait 300ms to see if anything happened.

![Without Touch States][Without Touch States]

Without Touch State ([code][4])

![With Touch States][With Touch States]

With Touch State ([code][5])

**However, if you want to make them actually respond immediately, you can go one 
step further.**

Using a JavaScript function called `fasttap` or `fastclick`, you can fully
remove that 300ms delay from your buttons. This, along with the active states,
will make your websites feel speedy as heck.

For more information on `fasttap`, read [this article by Google][6] or, for a
ready-made implementation, check out [this repo on Github][7].
...
![Without Momentum Scrolling][Without Momentum Scrolling]

No Momentum Scrolling ([code][8])

![With Momentum Scrolling][With Momentum Scrolling]

With Momentum Scrolling ([code][9])
...
in the [latest version of Mobile Safari on iOS 7][10].

One way to get around this is to create a class that adds `overflow-scrolling:
touch` to the container and then use JavaScript to only apply that class when
that container is visible.

On Android 4, you don’t even need this property. Every scrollable container has 
momentum scrolling.
...
JavaScript libraries you can try instead. Filament Group’s [Overthrow][11] and
[iScroll][12] work pretty well.
...
**Don’t worry, there are some great resources that make this stuff a lot easier!**
