# Руководство по ощущаемой производительности для новичков: 4 способа сделать мобильный вебсайт удобным как нативное приложение

> Примечание редактора: длина этой статьи ≈3000 слов. В ней рассмотрено 
множество аспектов влияющих на ощущаемую производительность мобильных вебсайтов, 
а также практических решений для ускорения работы сайта. Вкратце, нас интересует 
не насколько быстрым является вебсайт, а насколько быстрым он кажется 
пользователям.
...
Однако некоторые заказчики хотят чтобы сайт был так же удобен, как нативное 
приложение. Достижение такого впечатления остаётся задачей повышенной сложности.

В большинстве случаев когда пользователь говорит, что вебсайт *похож на 
...
медленным и тяжёлым вебсайтам. Сложилось мнение, что создать 
...
Несмотря на позицию команды Facebook, создание производительных вебсайтов не 
является невозможным. Это сложная задача, но она не выходит за рамки наших 
...
## Ощущаемая производительность и фактическая производительность

Хотя улучшение фактической производительности важно, как оказалось, для 
конечного пользователя она не играет такую уж большую роль если он не чувствует 
улучшение. 
...
который бы производил впечатление быстрого сайта.**

Единственное что имеет значение — это как пользователь воспринимает скорость 
работы вашего вебсайта. Любое фактическое ускорение работы сайта является не 
...
производительность сайта сию минуту?
...
вебсайте — добавить состояние `active` на мобильных устройствах. 
...
осуществить двойной тап). Потому они ждут чуть меньше трети секунды, чтобы 
проверить что пользователь будет делать дальше и если он ничего не делает, 
...
Тем не менее, на большинстве мобильных вебсайтов, в том числе и нескольких, 
...
визуальное подтверждение того, что что-то происходит.**

Для визуального обозначения нажатия на кнопку или ссылку есть замечательный 
признак — это состояние `:active`. Мы постоянно используем его для браузеров 
настольных компьютеров. 

К сожалению, ни iOS, ни Android не обращают внимания на этот псевдокласс когда 
происходит касание по кнопке или ссылке на мобильном устройстве. Чтобы сделать 
возможным состояние активности, нужно добавить на страницу простое событие 
JavaScript:
...
обратной прокрутки в верхнюю часть страницы посредством касания по строке 
...
Для Android 4 применять это свойство нет надобности. Для каждого контейнера с 
прокруткой по умолчанию используется прокрутка с ускорением.
...
Одно из наиболее заметных отличий между вебсайтами и приложениями состоит в 
...
Разработчики вебсайтов вынуждены довольствоваться анимацией на основе JavaScript, 
...
анимации и о том, насколько сильно некачественная анимация может навредить 
впечатлению от использования приложения.

Что интересно, он написал статью о разработке нативных приложений. Таким образом, 
она является отличным образцом для подражания в нашей миссии создания 
естественной анимации для вебсайтов. 
...
стремиться к такой скорости для всей анимации, напрямую связанной с 
взаимодействием с пользователем.

**2. Всё остальное должно отвечать не позднее чем через 100 мс.** Это число 
отображает психологическую грань, за которой всё кажется медленным. Всё, что 
...
беспокойство о таких вещах как расчёт продолжительности анимации? 
...
настоятельно рекомендую использовать и подстраивать её под ваш проект. 
...
**Дальше, как упоминал ранее, я расскажу как можно избежать использования 
...
Конечно же, повторяющаяся длительная анимация может быть раздражающей, так что 
используйте этот приём умеренно. Это хороший совет для большинства анимации.

## 4. Используйте естественные жесты

Одно из преимуществ приложений над вебсайтами — приспосабливание жестов, которые 
легко осваиваются пользователями сенсорных устройств.
...
На большинстве вебсайтов возможен только тап по объектам. Разработчики избегают 
...
мобильном сайте imgur.com][24]. Когда будете его просматривать, проверьте что 
можно сделать используя свайп.
...
нативных приложений и вебсайтов. Мы к этому ещё не пришли, но чем больше мы 
прилагаем усилий чтобы все наши вебсайты создавали впечатление разработанных с 
учётом потребностей пользователя, тем скорее этот день наступит. 

**Мне кажется что текущая тенденция к концентрации на производительности — это 
прекрасно, но не стоит забывать, что наши пользователи — не машины.**

Им все равно сколько запросов делает ваш вебсайт или как быстро происходит 
отрисовка страницы. Однако для них имеет значение то, на что эти факторы влияют — 
...
пользователь не почувствует его скорость.**
